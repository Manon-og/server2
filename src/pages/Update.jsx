...
const Update = ({ handleCloseDialog, selectedEmployeeId }) => {
  ...
  // const selectedEmployeeId = location.pathname.split("/")[2]

  useEffect(() => {

    const fetchEmployee = async () => {
      try {
        const res = await axios.get(`http://localhost:8800/employees/${selectedEmployeeId}`);
  ...
  const handleClick = async e => {
    ...
    const confirmMessage = `Are you sure you want to update the employee with the following details?\n
    First Name: ${employee.firstname}\n
    Middle Name: ${employee.middlename}\n
    Last Name: ${employee.lastname}\n
    Address Line: ${employee.addressline}\n
    ...
    Status: ${statusInput}\n
    Employee Type: ${emptypeInput}`;
  
    if (window.confirm(confirmMessage)) {
      // e.preventDefault();
      try{
        await axios.put("http://localhost:8800/employees/"+ selectedEmployeeId, employee) 
  ...
  return (
    <div className='form'> 
      ...
      <button className="add" onClick={(e) => handleCombinedClick(e)}>Add employee</button>
          <button className="add" onClick={(e) => handleCloseDialog(e)}>Cancel</button>
      </div>
    </div>
  )
}
